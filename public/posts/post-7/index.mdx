...
**즉, 모달 conten 내부의 인풋 값이 바뀌면 Zustand는 변경을 감지를 해 리렌더링을 시켜버린다.** 그래서 인풋 값이 유지되지 않고 날라가는 이슈가 발생했다.
...
useCallback이나 useMemo를 써봤지만, 결국 conten 내부 값이 바뀌기 때문에 리렌더링이 일어나는 것은 똑같았다.
